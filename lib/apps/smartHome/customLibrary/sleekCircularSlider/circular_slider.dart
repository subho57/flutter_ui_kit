...
import 'package:mighty_ui_kit/apps/smartHome/customLibrary/sleekCircularSlider/slider_animations.dart';
import 'package:mighty_ui_kit/apps/smartHome/customLibrary/sleekCircularSlider/utills.dart';
...
class SleekCircularSlider extends StatefulWidget {
  ...
  const SleekCircularSlider({Key key, this.initialValue = 5, this.min = 0, this.max = 30, this.appearance = defaultAppearance, this.onChange, this.onChangeStart, this.onChangeEnd, this.innerWidget})
...
class _SleekCircularSliderState extends State<SleekCircularSlider> with SingleTickerProviderStateMixin {
  ...
  void _setupPainter({bool counterClockwise = false}) {
    ...
    _currentAngle = calculateAngle(startAngle: _startAngle, angleRange: _angleRange, selectedAngle: _selectedAngle, defaultAngle: defaultAngle, counterClockwise: counterClockwise);

    _painter = _CurvePainter(startAngle: _startAngle, angleRange: _angleRange, angle: _currentAngle < 0.5 ? 0.5 : _currentAngle, appearance: widget.appearance);
  ...
  Widget _buildRotatingPainter({double rotation, Size size}) {
    if (rotation != null) {
      return Transform(transform: Matrix4.identity()..rotateZ((rotation) * 5 * math.pi / 6), alignment: FractionalOffset.center, child: _buildPainter(size: size));
    } else {
  ...
  Widget _buildPainter({Size size}) {
    return CustomPaint(painter: _painter, child: Container(width: size.width, height: size.height, child: _buildChildWidget()));
  }
  ...
  bool _onPanDown(Offset details) {
    ...
    final angleWithinRange = isAngleWithinRange(
        startAngle: _startAngle, angleRange: _angleRange, touchAngle: coordinatesToRadians(_painter.center, position), previousAngle: _currentAngle, counterClockwise: widget.appearance.counterClockwise);
    if (!angleWithinRange) {
      return false;
    }
